...
if st.session_state.current_page == "intro":
    ...
    이 곳은 우리가 평소에 마주할 수 있는 위험한 상황에 대처하는 방법을 배우기 위한 곳입니다.
    
    ### 🎯 어떻게 하나요?
                
    1. 왼쪽 메뉴에서 연습할 문제를 고르세요.
    2. 우리가 대처해야할 상황에 대한 설명을 읽으세요.
    3. 아래에 안내되어있는 안내글을 모두 읽으세요. (안내글에는 정확한 정보와 틀린 정보가 섞여있습니다.)
    4. 챗봇과 대화를 하며 정보를 얻으세요.
    5. 챗봇이 제시한 정보를 바탕으로 답을 고르세요. (시간제한 5분)
...
아래의 안내글 {len(TASKS[st.session_state.selected_task]["cards"])}개를 보고나서, 챗봇이랑 대화를 하고나서, 답을 고르세요. 맞는 안내글도 있고, 틀린 안내글도 있습니다.

(제한시간 5분, 채팅을 하나 주고받을 때마다, 남은 시간이 보여지게 업데이트됩니다.)</p><h3 style='color: {"#FF0000" if time_left < 60 else "#FFFFFF"};'>⏰ 남은 시간: {minutes:02d}:{seconds:02d}</h3></div>""", unsafe_allow_html=True)
